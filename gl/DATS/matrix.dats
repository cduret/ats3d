...
implement{a} vec3_vt_create(x, y, z) = array_ptr_new<a> $arrsz(x, y, z)

implement{a} vec3_vt_delete(v) = array_ptr_delete<a> v

implement{a} vec3_vt_add(v1, v2) = vec3_vt_create(x, y, z) where {
  macdef AT(x, y) = array_ptr_get_elt<a>(,(x), ,(y))
  val x = add(AT(v1,0), AT(v2,0))
  val y = add(AT(v1,1), AT(v2,1))
  val z = add(AT(v1,2), AT(v2,2))
  val () = vec3_vt_delete<a> v1
  val () = vec3_vt_delete<a> v2
}

implement{a} vec3_vt_substract(v1, v2) = vec3_vt_create(x, y, z) where {
  macdef AT(x, y) = array_ptr_get_elt<a>(,(x), ,(y))
  val x = sub(AT(v1,0), AT(v2,0))
  val y = sub(AT(v1,1), AT(v2,1))
  val z = sub(AT(v1,2), AT(v2,2))
  val () = vec3_vt_delete<a> v1
  val () = vec3_vt_delete<a> v2
}

implement{a} vec3_vt_multiply(v1, v2) = vec3_vt_create(x, y, z) where {
  macdef AT(x, y) = array_ptr_get_elt<a>(,(x), ,(y))
  val x = mul(AT(v1,0), AT(v2,0))
  val y = mul(AT(v1,1), AT(v2,1))
  val z = mul(AT(v1,2), AT(v2,2))
  val () = vec3_vt_delete<a> v1
  val () = vec3_vt_delete<a> v2
}

implement{a} vec3_vt_negate(v) = vec3_vt_create(x, y, z) where {
  macdef AT(x, y) = array_ptr_get_elt<a>(,(x), ,(y))
  val x = neg(AT(v, 0))
  val y = neg(AT(v, 1))
  val z = neg(AT(v, 2))
  val () = vec3_vt_delete<a> v
}

implement{a} vec3_vt_scale(v1, s) = vec3_vt_create(x, y, z) where {
  macdef AT(x, y) = array_ptr_get_elt<a>(,(x), ,(y))
  val x = mul(AT(v1,0), s)
  val y = mul(AT(v1,1), s)
  val z = mul(AT(v1,2), s)
...
implement{a} vec3_vt_normalize(v) = norm(v) where {
  val zero = lit<int>(0)
  val one = lit<int>(1)
  macdef eq_zero (x) = eq<a>(,(x), zero)
  macdef eq_one (x) = eq<a>(,(x), one)
  macdef AT(x, y) = array_ptr_get_elt<a>(,(x), ,(y))
  fun norm(v: vector3_vt(a)): vector3_vt(a) = let
    val len = vec3_vt_length v
  in
    if eq_zero(len) then let
      val () = vec3_vt_delete<a> v
    in vec3_vt_create<a>(zero, zero, zero) end
    else if eq_one(len) then v
    else let 
      val r = vec3_vt_create<a>(div(AT(v,0), len), div(AT(v,1), len), div(AT(v,2), len))
      val () = vec3_vt_delete<a> v
    in r end
  end
}

implement{a} vec3_vt_cross(v1, v2) = vec3_vt_create(x, y, z) where {
  macdef AT(x, y) = array_ptr_get_elt<a>(,(x), ,(y))
  val x = sub(mul(AT(v1,1),AT(v2,2)), mul(AT(v1,2),AT(v2,1)))
  val y = sub(mul(AT(v1,2),AT(v2,0)), mul(AT(v1,0),AT(v2,2)))
  val z = sub(mul(AT(v1,0),AT(v2,1)), mul(AT(v1,1),AT(v2,0)))
  val () = vec3_vt_delete<a> v1
  val () = vec3_vt_delete<a> v2
}

implement{a} vec3_vt_length(v) = sqrt(add(x_2, add(y_2, z_2))) where {
  macdef AT(x, y) = array_ptr_get_elt<a>(,(x), ,(y))
  val x_2 = mul(AT(v,0), AT(v,0))
  val y_2 = mul(AT(v,1), AT(v,1))
  val z_2 = mul(AT(v,2), AT(v,2))
}

implement{a} vec3_vt_dot(v1, v2) = add(x, add(y, z)) where {
  macdef AT(x, y) = array_ptr_get_elt<a>(,(x), ,(y))
  val x = mul(AT(v1,0), AT(v2,0))
  val y = mul(AT(v1,1), AT(v2,1))
  val z = mul(AT(v1,2), AT(v2,2))
...
implement{a} vec3_vt_direction(v1, v2) = vec3_vt_create(x, y, z) where {
  macdef AT(x, y) = array_ptr_get_elt<a>(,(x), ,(y))
  val v0 = vec3_vt_substract(v1, v2)
  val len = vec3_vt_length v0
  val x = mul(AT(v0,0), len)
  val y = mul(AT(v0,1), len)
  val z = mul(AT(v0,2), len)
  val () = vec3_vt_delete<a> v0
}

implement{a} vec3_vt_lerp(v1, v2, lerp) = vec3_vt_create(x, y, z) where {
  macdef AT(x, y) = array_ptr_get_elt<a>(,(x), ,(y))
  val x = add(AT(v1,0), mul(lerp, sub(AT(v2,0), AT(v1,0))))
  val y = add(AT(v1,1), mul(lerp, sub(AT(v2,1), AT(v1,1))))
  val z = add(AT(v1,2), mul(lerp, sub(AT(v2,2), AT(v1,2))))
  val () = vec3_vt_delete<a> v1
  val () = vec3_vt_delete<a> v2
}

implement{a} vec3_vt_dist(v1, v2) = sqrt( add(x_2, add(y_2, z_2)) ) where {
  macdef AT(x, y) = array_ptr_get_elt<a>(,(x), ,(y))
  val v0 = vec3_vt_substract(v1,v2)
  val x_2 = mul(AT(v0,0), AT(v0,0))
  val y_2 = mul(AT(v0,1), AT(v0,1))
  val z_2 = mul(AT(v0,2), AT(v0,2))
  val () = vec3_vt_delete<a> v0
}

implement{a} vec4_create(x, y, z, w) = array_make_view_ptr(pf|p) where {
  val (_, pf|p, sz) = $arrsz {a}(x, y, z, w)
}

(* ---------- VIEWTYPE ------------ *)

implement{a} vec4_vt_create(x, y, z, w) = array_ptr_new<a> $arrsz(x, y, z, w)

implement{a} vec4_vt_delete(v) = array_ptr_delete<a> v

implement{a} mat3_create (m11, m12, m13, m21, m22, m23, m31, m32, m33) = 
  array_make_view_ptr(pf|p) where {
  val (_, pf|p, sz) = $arrsz {a}(m11, m12, m13, m21, m22, m23, m31, m32, m33)
}
...
implement{a} mat3_vt_create (m11, m12, m13, m21, m22, m23, m31, m32, m33) = 
  array_ptr_new<a> $arrsz(m11, m12, m13, m21, m22, m23, m31, m32, m33)

implement{a} mat3_vt_delete(m) = array_ptr_delete<a> m
...
implement{a} mat4_create(m11, m12, m13, m14, m21, m22, m23, m24,
  ...
  array_make_view_ptr(pf|p) where {
  val (_, pf|p, sz) = $arrsz {a}(m11, m12, m13, m14, m21, m22, m23, m24,
                                 m31, m32, m33, m34, m41, m42, m43, m44)
}
...
implement{a} mat4_vt_create(m11, m12, m13, m14, m21, m22, m23, m24,
                         m31, m32, m33, m34, m41, m42, m43, m44) = 
  array_ptr_new<a> $arrsz(m11, m12, m13, m14, m21, m22, m23, m24,
                                 m31, m32, m33, m34, m41, m42, m43, m44)

implement{a} mat4_vt_delete(m) = array_ptr_delete<a> m

implement{a} mat4_vt_identity() = mat4_vt_create(one, zero, zero, zero,
                                           zero, one, zero, zero,
                                           zero, zero, one, zero,
                                           zero, zero, zero, one) where {
  val zero = lit<int>(0)
...
implement{a} mat4_vt_transpose(m) = let
  macdef AT(x, y) = array_ptr_get_elt<a>(,(x), ,(y))
  val r = mat4_vt_create(AT(m,0), AT(m,4), AT(m,8), AT(m,12),
                      AT(m,1), AT(m,5), AT(m,9), AT(m,13),
                      AT(m,2), AT(m,6), AT(m,10), AT(m,14),
                      AT(m,3), AT(m,7), AT(m,11), AT(m,15))
  val () = mat4_vt_delete<a> m
in
  r
end

implement{a} mat4_vt_determinant(m) = x where {
  macdef AT(x, y) = array_ptr_get_elt<a>(,(x), ,(y))
...
implement{a} mat4_vt_inverse(m) = inverse(m) where {
  macdef AT(x, y) = array_ptr_get_elt<a>(,(x), ,(y))

  fun inverse(m: matrix4_vt(a)): matrix4_vt(a) = let
    val a00 = AT(m,0) val a01 = AT(m,1) val a02 = AT(m,2) val a03 = AT(m,3)
    val a10 = AT(m,4) val a11 = AT(m,5) val a12 = AT(m,6) val a13 = AT(m,7)
    val a20 = AT(m,8) val a21 = AT(m,9) val a22 = AT(m,10) val a23 = AT(m,11)
    val a30 = AT(m,12) val a31 = AT(m,13) val a32 = AT(m,14) val a33 = AT(m,15)
    ...
    macdef eq_zero (x) = eq<a>(,(x), zero)
  in
    if eq_zero(det) then mat4_vt_create<a>(zero, zero, zero, zero,
                                           zero, zero, zero, zero,
                                           zero, zero, zero, zero,
                                           zero, zero, zero, zero)
    else let
      ...
      val m33 = mul(add(sub(mul<a>(a20, b03), mul<a>(a21, b01)), mul<a>(a22, b00)), inv_det)
    in
      mat4_vt_create<a>(m00, m01, m02, m03,
                        m10, m11, m12, m13, 
                        m20, m21, m22, m23, 
                        m30, m31, m32, m33)
    end
...
implement{a} mat4_vt_multiply(m1, m2) = mat4_vt_create(c00, c01, c02, c03,
                                                 c10, c11, c12, c13,
                                                 c20, c21, c22, c23,
                                                 c30, c31, c32, c33) where {
  macdef AT(x, y) = array_ptr_get_elt<a>(,(x), ,(y))
  ...
  val () = mat4_vt_delete<a> m1
  val () = mat4_vt_delete<a> m2
}

implement{a} mat4_vt_multiply_vec3(m, v) = vec3_vt_create(x1, y1, z1) where {
  macdef AT(x, y) = array_ptr_get_elt<a>(,(x), ,(y))

  val x1 = add(add(add(mul(AT(v,0), AT(m,0)), mul(AT(v,1), AT(m,4))), mul(AT(v,2), AT(m,8))), AT(m,12))
  val y1 = add(add(add(mul(AT(v,0), AT(m,1)), mul(AT(v,1), AT(m,5))), mul(AT(v,2), AT(m,9))), AT(m,13))
  val z1 = add(add(add(mul(AT(v,0), AT(m,2)), mul(AT(v,1), AT(m,6))), mul(AT(v,2), AT(m,10))), AT(m,14))

  val () = mat4_vt_delete<a> m
  val () = vec3_vt_delete<a> v
}

implement{a} mat4_vt_multiply_vec4(m, v) = vec4_vt_create(x1, y1, z1, w1) where {
  macdef AT(x, y) = array_ptr_get_elt<a>(,(x), ,(y))

  val x1 = add(add(add(mul(AT(v,0), AT(m,0)), mul(AT(v,1), AT(m,4))), mul(AT(v,2), AT(m,8))), AT(m,12))
  val y1 = add(add(add(mul(AT(v,0), AT(m,1)), mul(AT(v,1), AT(m,5))), mul(AT(v,2), AT(m,9))), AT(m,13))
  val z1 = add(add(add(mul(AT(v,0), AT(m,2)), mul(AT(v,1), AT(m,6))), mul(AT(v,2), AT(m,10))), AT(m,14))
  val w1 = add(add(add(mul(AT(v,0), AT(m,3)), mul(AT(v,1), AT(m,7))), mul(AT(v,2), AT(m,11))), AT(m,15))

  val () = mat4_vt_delete<a> m
  val () = vec4_vt_delete<a> v
}

implement{a} mat4_vt_translate(m, v) = mat4_vt_create(a00, a01, a02, a03,
                                                a10, a11, a12, a13,
                                                a20, a21, a22, a23,    
                                                b30, b31, b32, b33) where {
  macdef AT(x, y) = array_ptr_get_elt<a>(,(x), ,(y))
  ...
  val b30 = add(add(add(mul(a00, AT(v,0)), mul(a10, AT(v,1))), mul(a20, AT(v,2))), a30)
  val b31 = add(add(add(mul(a01, AT(v,0)), mul(a11, AT(v,1))), mul(a21, AT(v,2))), a31)
  val b32 = add(add(add(mul(a02, AT(v,0)), mul(a12, AT(v,1))), mul(a22, AT(v,2))), a32)
  val b33 = add(add(add(mul(a03, AT(v,0)), mul(a13, AT(v,1))), mul(a23, AT(v,2))), a33)

  val () = mat4_vt_delete<a> m
  val () = vec3_vt_delete<a> v
}

implement{a} mat4_vt_scale(m, v) = mat4_vt_create(a00, a01, a02, a03,
                                         a10, a11, a12, a13,
                                         a20, a21, a22, a23,     
                                         a30, a31, a32, a33) where {
  macdef AT(x, y) = array_ptr_get_elt<a>(,(x), ,(y))
  ...
  val a00 = mul(AT(m,0), x) val a01 = mul(AT(m,1), x) val a02 = mul(AT(m,2), x) val a03 = mul(AT(m,3), x)
  val a10 = mul(AT(m,4), y) val a11 = mul(AT(m,5), y) val a12 = mul(AT(m,6), y) val a13 = mul(AT(m,7), y)
  val a20 = mul(AT(m,8), z) val a21 = mul(AT(m,9), z) val a22 = mul(AT(m,10), z) val a23 = mul(AT(m,11), z)
  val a30 = AT(m,12) val a31 = AT(m,13) val a32 = AT(m,14) val a33 = AT(m,15)
...
implement {a} mat4_vt_rotate(m, angle, axis) = mat4_vt_create(c00, c01, c02, c03,
                                                        c10, c11, c12, c13,
                                                        c20, c21, c22, c23,
                                                        c30, c31, c32, c33) where {
  macdef AT(x, y) = array_ptr_get_elt<a>(,(x), ,(y))
  ...
  val () = mat4_vt_delete<a> m
  val () = vec3_vt_delete<a> axis
}

implement{a} mat4_vt_frustum(left, right, bottom, top, near, far) = 
          mat4_vt_create(a00, zero, zero, zero,
                         zero,  a11, zero, zero,
                         a20, a21, a22, neg(one),
                         zero, zero, a32, zero) where {
  val zero = lit<int>(0)
...
implement{a} mat4_vt_perspective(fovy, aspect, near, far) = 
        mat4_vt_frustum(neg(right), right, neg(top), top, near, far) where {
  val PI = lit<double><a>(3.14159265358979)
  val top = mul(near, tan(div(mul(fovy, PI), lit<int>(360))))
  val right = mul(top, aspect)
}

implement{a} mat4_vt_ortho(left, right, bottom, top, near, far) = 
        mat4_vt_create(a00, zero, zero, zero,
                       zero,  a11, zero, zero,
                       zero, zero, a22, zero,
                       a30,  a31,  a32,  one) where {
...
implement{a} mat4_vt_lookat(eye, center, up) = mat4_vt_create(x0,  y0,  z0,  zero,
                                                        x1,  y1,  z1,  zero,
                                                        x2,  y2,  z2,  zero,
                                                        a30, a31, a32, one) where {
  macdef AT(x, y) = array_ptr_get_elt<a>(,(x), ,(y))
  ...
  val z = vec3_vt_direction(eye, center)
  val z0 = AT(z,0) val z1 = AT(z,1) val z2 = AT(z,2)
  val x = vec3_vt_normalize(vec3_vt_cross(up, z))
  val x0 = AT(x,0) val x1 = AT(x,1) val x2 = AT(x,2)
  val y = vec3_vt_normalize(vec3_vt_cross(vec3_vt_create(z0, z1, z2), x))
  val y0 = AT(y,0) val y1 = AT(y,1) val y2 = AT(y,2)


  val a30 = sub(add(mul(x0, eye0), mul(x1, eye1)), mul(x2, eye2))
  val a31 = sub(add(mul(y0, eye0), mul(y1, eye1)), mul(y2, eye2))
  val a32 = sub(add(mul(z0, eye0), mul(z1, eye1)), mul(z2, eye2))

  val () = vec3_vt_delete<a> y
}
